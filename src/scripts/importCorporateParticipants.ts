...
const SQL_PATH = path.resolve(__dirname, '../../../docs/migrated-data/corporate-users-corporate.sql');
...
export async function importCorporateParticipants(options: { dryRun?: boolean } = {}): Promise<ImportSummary> {
  ...
  try {
    const [userRows] = await connection.query<mysql.RowDataPacket[]>(
      "SELECT id FROM users WHERE role IN ('corporate', 'cto')"
    );
    ...
    for (const row of parsed.sort((a, b) => a.legacyId - b.legacyId)) {
      ...
      if (!validUserIds.has(row.corporateUserId)) {
        ...
        summary.errors.push({
          legacyId: row.legacyId,
          email: row.email,
          error: `Corporate user ${row.corporateUserId} not found`,
        });
        continue;
      }
    ...
    if (maxId > 0) {
      await connection.query(`ALTER TABLE participants AUTO_INCREMENT = ${maxId + 1}`);
    }
  } finally {
...
async function main() {
  ...
  const summary = await importCorporateParticipants({ dryRun });
